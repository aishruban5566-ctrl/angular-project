...
export class OriginBranchComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.items = [
      { id: 1, code: 'OB001', name: 'Chennai Branch', address: 'Harbour Rd', city: 'Chennai', active: true },
      { id: 2, code: 'OB002', name: 'Mumbai Branch', address: 'Dock Area', city: 'Mumbai', active: true }
    ];
  }

  openAdd() { this.isEditing = false; this.selectedId = null; this.form.reset({ active: true }); this.isModalOpen = true; }
  openEdit(i: OriginBranch) { this.isEditing = true; this.selectedId = i.id; this.form.setValue({ code: i.code, name: i.name, address: i.address || '', city: i.city || '', active: !!i.active }); this.isModalOpen = true; }
  ...
  filtered() {
    ...
    return this.items.filter(i => i.code.toLowerCase().includes(q) || i.name.toLowerCase().includes(q) || (i.city || '').toLowerCase().includes(q) || (i.address || '').toLowerCase().includes(q));
  }
